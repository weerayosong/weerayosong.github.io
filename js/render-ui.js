                ...
                <div class="p-8 md:p-10 flex flex-col md:flex-row md:items-start justify-between gap-8 z-10">
                    ...
                    <div class="w-full md:w-80 shrink-0 space-y-6">
                        ...
                        <div class="pt-6 flex gap-6 text-sm font-medium border-t border-gray-50 dark:border-slate-800/60">
                            <a href="${project.githubLink}" target="_blank" class="text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors duration-200 flex items-center gap-2">
                                <i class="fab fa-github text-lg"></i> Code
                            </a>
                            ${
                                project.previewLink
                                    ? `
                                <a href="${project.previewLink}" target="_blank" class="text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors duration-200 flex items-center gap-2">
...
export function renderCertificates() {
    ...
    certsData.forEach((cert) => {
        ...
        card.target = "_blank";
        card.className =
            "relative overflow-hidden rounded-md border border-slate-800 bg-gradient-to-br from-slate-900 to-slate-950 hover:border-slate-400 transition-all duration-300 shadow-[inset_0_1px_0_0_rgba(255,255,255,0.1),0_10px_30px_rgba(0,0,0,0.5)] group cursor-pointer block h-28";
        card.innerHTML = `
            ...
            <div class="relative z-10 p-4 flex items-center gap-6 w-full h-full">
                <div class="rounded-md w-10 h-10 border border-slate-700 bg-slate-900/90 flex items-center justify-center shrink-0 shadow-[inset_0_1px_0_0_rgba(255,255,255,0.2)] transform rotate-45 group-hover:border-slate-400 transition-colors">
                    <i class="${cert.icon} text-slate-400 text-2xl transform -rotate-45"></i>
                </div>
                <div class="flex-1 min-w-0">
                    ...
                    <p class="text-slate-400 text-xs font-semibold uppercase tracking-wider mt-1 drop-shadow-md truncate">${cert.issuer}</p>
                </div>
                <div class="shrink-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    <i class="fas fa-external-link-alt text-sm text-slate-400"></i>
                </div>
            </div>
        `;
